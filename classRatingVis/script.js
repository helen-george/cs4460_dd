...
d3.csv("data.csv").then(data => {
    ...
    // Define a color scale using YlOrRd (Yellow to Red)
    const colorScale = d3.scaleSequential()
        .domain([1, 9]) // Class rating range
        .interpolator(d3.interpolateYlOrRd); // Yellow to Red color scale
    ...
    const legendScale = d3.scaleLinear()
        .domain([1, 9])
        .range([d3.interpolateYlOrRd(0), d3.interpolateYlOrRd(1)]);
